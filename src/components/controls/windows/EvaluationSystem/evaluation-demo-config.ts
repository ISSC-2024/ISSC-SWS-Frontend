 ...
 */
export const checkDemoCondition = (evaluationData: any): DemoConfig | null => {
  if (!DEMO_ENABLED) return null

  // 检查专家ID
  const hasTargetExpert = evaluationData.experts?.some((expert: any) => expert.id === DEMO_CONFIG.condition.expertId)
  ...
  if (!hasIndicators && DEMO_CONFIG.condition.useDefaultIndicators) return null
 ...
 * @returns 演示条件的文字描述
 */
export const getDemoConditionDescription = (): string => {
  if (!DEMO_ENABLED) return '演示功能已禁用'

  return `演示触发条件：
- 专家ID为 ${DEMO_CONFIG.condition.expertId}
- 使用默认评价指标体系
- 智能评价工具只选中"多智能体辩论评价"`
}
